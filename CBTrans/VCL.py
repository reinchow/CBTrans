...
class EnhancedLoss(nn.Module):
    def __init__(self, margin=0.2):
        ...
        self.margin = margin
        self.linear_layer = nn.Linear(1024, 680)
        #self.linear_layer1 = nn.Linear(1280, 3)
        # 可学习的损失权重
        self.image_loss_weight = nn.Parameter(torch.tensor(0.2))
        self.text_loss_weight = nn.Parameter(torch.tensor(0.2))
        self.label_loss_weight = nn.Parameter(torch.tensor(0.6))
    ...
    def forward(self, image_output, text_output, text_target,logits,labels): #encoder_out, scores, targets,logits,labels
        ...
        image_mapped = self.linear_layer(image_output.reshape(-1, 1024))
        #image_mapped1 = self.linear_layer1(image_output.reshape(-1, 1280))
        selected_indices = torch.randperm(image_mapped.size(0))[:680].to("cuda:1")
