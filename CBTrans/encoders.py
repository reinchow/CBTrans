...
from attention import MultiHeadAttention1
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model=1024, d_k=128, d_v=128, h=8, d_ff=2048, dropout=.1, identity_map_reordering=False,
                 attention_module=None, attention_module_kwargs=None):
        super(EncoderLayer, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        self.mhatt = MultiHeadAttention1(d_model, d_k, d_v, h, dropout, identity_map_reordering=identity_map_reordering,
    ...
    def forward(self, queries, keys, values, relative_pos, attention_mask, attention_weights=None, pos=None):
        ...
        att2 = self.lp(att1)
        att2= self.lnorm2(att1 + self.dropout2(att2))
...
class MultiLevelEncoder(nn.Module):
    def __init__(self, N, d_model=1024, d_k=128, d_v=128, h=8, d_ff=2048, dropout=.1,
    ...
    def forward(self, input, attention_weights=None, pos=None):
        ...
        #print("f",flatten_relative_geometry_embeddings.device)
        device = input.device  # 获取输入的设备
        flatten_relative_geometry_embeddings = flatten_relative_geometry_embeddings.to(device)
